...
export const createVendor = async (req: Request, res: Response): Promise<Response> => {
    try{
        ...
        await pool.query('INSERT INTO Vendors(id, name, licence_no, registration_no, phone_no, email_id, website, fax_no, '
...
export const getMaxId = async (req: Request, res: Response): Promise<Response> => {
    try {
      const response: QueryResult = await pool.query('SELECT max(Id)+1 as maxId FROM Vendors ');  
...
export const getAllVendors = async (req: Request, res: Response): Promise<Response> => {
    try {
      const response: QueryResult = await pool.query('SELECT * FROM Vendors order by id asc');  
      return res.status(200).json(response.rows);
    } catch (error) {
      console.error(error);
      return res.status(500).json('Internal Server error');
...
export const getVendorById = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      const response: QueryResult = await pool.query('SELECT * FROM Vendors WHERE id = $1', [id]);
...
export const getVendorByName = async (req: Request, res: Response): Promise<Response> => {
    const name = "'"+parseInt(req.params.name) + "%'";  
    try {
      const response: QueryResult = await pool.query('SELECT * FROM Vendors WHERE name like $1', [name]);
      return res.json(response.rows);
    } catch (error) {
      console.error(error);
      return res.status(500).json('Internal Server error');
...
export const VerifyVendor = async (req: Request, res: Response): Promise<Response> => {
    const id = parseInt(req.params.id);  
    const {status, RejectedReason} = req.body; 
             
    try {
      const response: QueryResult = await pool.query('UPDATE Vendors SET IsVerified=$1, RejectedReason =$2 WHERE id = $3', [status, RejectedReason, id]);
      return res.json({
        message: 'Updated',
        Verify_Details: {
...
export const updateVendors = async (req: Request, res: Response): Promise<Response> => {
    const id = parseInt(req.params.id);
    console.error(req.body);
    const { name, licence_no, registration_no, phone_no, email_id, website, fax_no, 
        address, city, state, country, pincode} = req.body;  
    try {
      await pool.query('UPDATE Vendors SET name=$1, licence_no=$2, registration_no=$3, phone_no=$4, email_id=$5, website=$6, fax_no=$7, '
...
export const deleteVendor = async (req: Request, res: Response): Promise<Response> => {
    ...
    let inActive = 'N';     
    try {
      await pool.query('UPDATE Vendors SET IsActive=$1 WHERE id = $2', [inActive, id]);
