...
export const createCountry = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
     const citys = await getAllCountry(req, res);

     console.log(citys);
    //const id = 0;
    const { id, Country } = req.body;    
    const response: QueryResult =  await pool.query('INSERT INTO CountryMaster(id, Country) VALUES ($1, $2)', [id, Country ]);
...
export const updateCountry = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      ...
      const response: QueryResult =  await pool.query('UPDATE CountryMaster SET Country=$1 WHERE id = $2', [Country, id]);      
      return res.json({
        message: 'Updated'
      });
    } catch (error) {   
...
export const deleteCountry = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
      const id = parseInt(req.params.id);
      const response: QueryResult = await pool.query('DELETE FROM CountryMaster WHERE id = $1', [id]);
